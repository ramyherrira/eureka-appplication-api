.PHONY: validate install update phpcs phpcbf php81compatibility phpstan analyze tests testdox ci clean
...
php81compatibility: vendor/bin/phpstan build/reports/phpstan
	...
	@./vendor/bin/phpstan analyse --xdebug --configuration=./ci/php81-compatibility.neon --error-format=checkstyle > ./build/reports/phpstan/php81-compatibility.xml
...
behat: vendor/bin/behat
	$(call header,Running Behat Tests)
...
deploy: composer.lock
	$(call header,Deploy application)
	@git checkout master && git pull origin HEAD
	@make update
	@./bin/console --color deploy

ci: clean validate phpcs tests behat php81compatibility analyze
